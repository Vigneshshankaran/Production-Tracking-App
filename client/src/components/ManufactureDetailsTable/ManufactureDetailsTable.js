...
const ManufactureDetailsTable = () => {
  ...
  const [sweetFilterAnchorEl, setSweetFilterAnchorEl] = useState(null);
  const [deliveryStatusFilterAnchorEl, setDeliveryStatusFilterAnchorEl] = useState(null);
  ...
  const filteredData = (customerData || [])
    ...
    .filter(
      (customer) =>
        ...
        (deliveryStatusFilter === "All" ||
          customer.status === deliveryStatusFilter) 
    )
    ...
    const handleSweetFilterClick = (event) => {
      setSweetFilterAnchorEl(event.currentTarget);
    };
  
    const handleSweetFilterClose = (filter) => {
      setSweetFilterAnchorEl(null);
      setSweetFilter(filter);
    };
  
    const handleDeliveryStatusFilterClick = (event) => {
      setDeliveryStatusFilterAnchorEl(event.currentTarget);
    };
  
    const handleDeliveryStatusFilterClose = (filter) => {
      setDeliveryStatusFilterAnchorEl(null);
  ...
  return (
    <>
      ...
      <IconButton onClick={handleSweetFilterClick}>
        <FilterListIcon />
      </IconButton>
      <Menu
        anchorEl={sweetFilterAnchorEl} 
        keepMounted
        open={Boolean(sweetFilterAnchorEl)}
        onClose={() => handleSweetFilterClose(null)}
      >
        <MenuItem onClick={() => handleSweetFilterClose("All")}>All</MenuItem>
        <MenuItem onClick={() => handleSweetFilterClose("Main Sweets")}>
          Main Sweets
        </MenuItem>
        <MenuItem onClick={() => handleSweetFilterClose("Custom Sweets")}>
          Custom Sweets
        </MenuItem>
      </Menu>

  <IconButton onClick={handleDeliveryStatusFilterClick}>
  <FilterListIcon />
</IconButton>
<Menu
  anchorEl={deliveryStatusFilterAnchorEl}
  keepMounted
  open={Boolean(deliveryStatusFilterAnchorEl)}
  onClose={() => handleDeliveryStatusFilterClose(null)}
>
  <MenuItem onClick={() => handleDeliveryStatusFilterClose('All')}>All</MenuItem>
  <MenuItem onClick={() => handleDeliveryStatusFilterClose('Delivered')}>Delivered</MenuItem>
  <MenuItem onClick={() => handleDeliveryStatusFilterClose('Confirmed')}>Confirmed</MenuItem>
  <MenuItem onClick={() => handleDeliveryStatusFilterClose('In Progress')}>In Progress</MenuItem>
  <MenuItem onClick={() => handleDeliveryStatusFilterClose('Completed')}>Completed</MenuItem>
  <MenuItem onClick={() => handleDeliveryStatusFilterClose('Pending')}>Pending</MenuItem>
</Menu>
