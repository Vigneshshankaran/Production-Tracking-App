...
const CustomerDetailsTable = () => {
  ...
  const handleDeliveryStatusFilterClose = (filterValue) => {
    setDeliveryStatusFilterAnchorEl(null);
    if (filterValue !== null) {
      setDeliveryStatusFilter(filterValue);
    }
  };
  ...
  .filter(customer => 
    ...
    (customer.status || 'Pending') === deliveryStatusFilter 
  ) 
...
{/* Add the new delivery status filter menu */}
